...
describe BrokerRole, dbclean: :around_each do
  ...
  let(:saved_person) {FactoryBot.create(:core_person, addresses: [address])}
  let(:person0) {FactoryBot.create(:core_person)}
  let(:person1) {FactoryBot.create(:core_person)}
  let(:npn0) {"7775566"}
  let(:npn1) {"48484848"}
  ...
  describe ".new" do
    ...
    context "a broker registers" do
      let(:person)  { FactoryBot.build(:core_person) }
  ...
  describe BrokerRole, '.find', :dbclean => :around_each do
  ...
  describe BrokerRole, '.all', :dbclean => :around_each do
  ...
  describe BrokerRole, :dbclean => :around_each do
    ...
    context '#email returns work email' do
      person0= FactoryBot.create(:core_person)
